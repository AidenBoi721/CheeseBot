...
async def set_birthday(interaction: discord.Interaction, month: int, day: int, user: discord.User):
    ...
    if invoking_user.id != target_user.id and not invoking_user.guild_permissions.administrator:
        await interaction.response.send_message("❌ You can only set your own birthday unless you're a server admin.", ephemeral=True)
